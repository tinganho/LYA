// This code is auto generate. Don't edit it!

#include "types.h"
#include "diagnostics.h"

using namespace std;
using namespace Lya::lib::types;

namespace Lya::core::diagnostics {

    DiagnosticTemplate D::Capabilities_must_be_a_string_plural_select_etc = DiagnosticTemplate { "Capabilities must be a string 'plural', 'select', etc." };
    DiagnosticTemplate D::Could_not_find_an_extension_for_the_programming_language_0 = DiagnosticTemplate { "Could not find an extension for the programming language '{0}'." };
    DiagnosticTemplate D::Could_not_read_your_extension_file_0 = DiagnosticTemplate { "Could not read your extension file '{0}'." };
    DiagnosticTemplate D::Could_not_start_extension_server = DiagnosticTemplate { "Could not start extension server." };
    DiagnosticTemplate D::Duplicate_value_message_0 = DiagnosticTemplate { "Duplicate value message '{0}'." };
    DiagnosticTemplate D::Expected_0_but_got_1 = DiagnosticTemplate { "Expected '{0}', but got '{1}'." };
    DiagnosticTemplate D::Failed_validation_of_file_0_1 = DiagnosticTemplate { "Failed validation of file {0}: {1}." };
    DiagnosticTemplate D::Missing_field_0_in_your_extension_file_1 = DiagnosticTemplate { "Missing field '{0}' in your extension file '{1}'." };
    DiagnosticTemplate D::Plural_form_0_is_not_supported_in_language_1 = DiagnosticTemplate { "Plural form '{0}' is not supported in language '{1}'." };
    DiagnosticTemplate D::Unexpected_end_of_message = DiagnosticTemplate { "Unexpected end of message." };
    DiagnosticTemplate D::Unknown_capability_0_in_extension_file_1 = DiagnosticTemplate { "Unknown capability '{0}' in extension file '{1}'." };
    DiagnosticTemplate D::Unknown_command_0 = DiagnosticTemplate { "Unknown command '{0}'." };
    DiagnosticTemplate D::Unknown_command_flag_0 = DiagnosticTemplate { "Unknown command flag '{0}'." };
    DiagnosticTemplate D::Unknown_plural_category = DiagnosticTemplate { "Unknown plural category." };
    DiagnosticTemplate D::Unmatched_signature = DiagnosticTemplate { "Unmatched signature." };
    DiagnosticTemplate D::You_are_not_inside_a_LYA_project = DiagnosticTemplate { "You are not inside a LYA project." };
    DiagnosticTemplate D::You_cannot_run_several_commands = DiagnosticTemplate { "You cannot run several commands." };

} // Lya::core::diagnostics 
